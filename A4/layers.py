...
class DenseLayer(BaseLayer):
    ...
    def compute_gradient(self):
        ...
        # self.dw
        self.dw = np.dot(np.transpose(self._output_error_gradient), self.get_input_data())
        self.dw = np.transpose(self.dw)
        
        #self.db
        mSize = np.shape(self._output_error_gradient)
        one = np.ones((1,mSize[0]), dtype = int)
        self.db = np.dot(one , self._output_error_gradient)
        ...
        # print("dw: ", self.dw)
        # print("db: ", self.db)
        # print("_input_error_gradient: ", self._input_error_gradient)
    ...
    def update_weights(self, learning_rate):
        ...
        learning rate.
        """
        # YOUR CODE STARTS HERE (3) new_weights = current_weights − learning_rate ∗ gradient
