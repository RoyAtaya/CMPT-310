...
class NeuralNet:
    ...
    def compute_gradients(self):
        ...
        lossGradient = self.loss.get_input_error_gradient()
        for layer in reversed(range(length)):
            self._layers[layer].set_output_error_gradient(lossGradient)
            self._layers[layer].compute_gradient()
            lossGradient = self._layers[layer].get_input_error_gradient()
